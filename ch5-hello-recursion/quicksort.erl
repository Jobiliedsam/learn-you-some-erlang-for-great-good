...
%%% Problem 1: Process values that are equal to the pivot more than once
%%% Problem 2: Its need to transverse all the partitioned lists more than once when attaching them to the pivot
...
bestest_partition(_, [], {Smaller, Equal, Larger}, Acc) ->
    bestest_qsort(Smaller, Equal ++ bestest_qsort(Larger, Acc)); % Verify if its is the best way to remove transverse overhead concatenation
